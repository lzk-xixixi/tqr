...
class MHTempCtxAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None, att_l=False):
        ...
        if mask is not None:
            mask = mask.unsqueeze(1)
        n, m, _ = query.size()
        query = self.linears[0](query).view(n, m, self.h, self.d_k)
        key = self.linears[1](key).view(n, m, self.h, self.d_k)
        value = self.linears[2](value).view(n, m, self.h, self.d_k)

        x, self.attn = self.temp_ctx_attention(query, key, value, mask=mask,
                                               dropout=self.dropout)

        x = x.contiguous().view(n, m, self.h * self.d_k)
...
class TempCtxAttention(nn.Module):
    ...
    def ctx_attention(self, query, key, value, mask=None, dropout=None, att_l=False):
        ...
        if att_l:
            scores = torch.einsum('nhld,mhkd->nmhlk', [query, key]) / math.sqrt(d_k)
            scores = torch.mean(scores, -2)
        else:
            scores = torch.einsum('nhd,mhkd->nmhk', [query, key]) / math.sqrt(d_k)
        p_attn = F.softmax(scores, dim=-1)
        if dropout is not None:
            p_attn = dropout(p_attn)

        ctx_result = torch.einsum('nmhk,mhkd->nmhd', [p_attn, value])
    ...
    def forward(self, query, key, value, mask=None):
        ...
        try:
            query = self.linears[0](query).view(n, self.h, self.d_k)
        except:
            print(query.size())
            query = self.linears[0](query).view(n, self.h, self.d_k)

        key = self.linears[1](key).view(m, -1, self.h, self.d_k).transpose(1, 2)
        value = self.linears[2](value).view(m, -1, self.h, self.d_k).transpose(1, 2)
...
class ShiftTempAttention(Model):
    ...
    def forward(self, token_embed: torch.Tensor, temp_ctx_embed: torch.Tensor, date: Any) -> torch.Tensor:
        history_embeds = []
        token_embed = token_embed.unsqueeze(1).expand(-1, self.num_authors, -1)
        for i in range(self.num_shift):
            time_embed = [self.time_encoder.get_time_encoding(d, num_shift=i) for d in date]
            time_embed = torch.stack(time_embed, dim=0)
            time_embed = time_embed.unsqueeze(1).expand(-1, self.num_authors, -1)
            ...
            if i == self.num_shift - 1:
                ...
                temp_ctx_embed = self.temp_layer_norms[i](temp_ctx_embed_te)
            history_embeds.append(temp_ctx_embed_te)
        history_embed = torch.stack(history_embeds, dim=1)
