...
class StackExDataReader(DatasetReader):
    ...
    def text_to_instance(self, tokens: List[Token], id: str,
                         answerers: np.ndarray, date: pd.Timestamp, accept_usr: str,dates: Any, raiser_dates: Any) -> Instance:
        ...
        fields["dates"] = MetadataField(dates)
        fields["accept_usr"] = MetadataField(accept_usr)
        fields["raiser_dates"] = MetadataField(raiser_dates)
    ...
    def _read(self, file_path: str) -> Iterator[Instance]:
        ...
        for i, row in df.iterrows():
            ...
            raiser_dates = json.loads(row["raiser_dates"])
            if date < self.from_date: self.from_date = date
            if date > self.to_date: self.to_date = date
            if self.use_tokenizer:
                yield self.text_to_instance(
                    tokens, row["id"], answerers, date, accept_usrs,dates,raiser_dates
                )
            else:
                yield (
                    tokens, row["id"], answerers, date, accept_usrs,dates,raiser_dates
